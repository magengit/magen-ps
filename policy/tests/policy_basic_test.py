...
class TestRestApi(unittest.TestCase):
    ...
    def test_FullReset(self):
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                ...
                POLICY_TEMPLATE_GET_RESP_FINANCE).success)
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                server_urls.policy_template_url,
                POLICY_TEMPLATE_POST_REQ_ACCOUNTING,
                POLICY_TEMPLATE_GET_RESP_ACCOUNTING).success)
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                server_urls.policy_template_url,
                POLICY_TEMPLATE_POST_REQ_MARKETING,
                POLICY_TEMPLATE_GET_RESP_MARKETING).success)
    ...
    def test_Misc(self):
        ...
        self.assertTrue(response.text.find("Welcome to Policy") != -1)

        check_url = root_url + "check/"
        success, response = PolicyTestCommon.get_doc(check_url)
        self.assertTrue(success)
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertTrue((response.text == "Check success"))
    ...
    def test_DctxDB(self):
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                ...
                dumps(dctx_dict),
                DCTX_GET_RESP_COMPLIANT_IPAD).success)
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                dctx_server_urls.dctx_one_state_base_url,
                dumps(dctx_dict),
                DCTX_GET_RESP_NONCOMPLIANT_IPAD).success)
        message = DCTX_POST_REQ_COMPLIANT_IPAD
        dctx_dict = loads(message)
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                dctx_server_urls.dctx_one_state_base_url,
                dumps(dctx_dict),
                DCTX_GET_RESP_COMPLIANT_MAC).success)
        ...
        self.assertTrue(RestClientApis.http_delete_and_get_check(
            dctx_server_urls.dctx_one_state_by_device_id_url.format(ipad1_device_id)).success)
        get_resp_object = RestClientApis.http_get_and_check_success(
            dctx_server_urls.dctx_all_states_url)
        states = get_resp_object.json_body['response']['device_states']
        ...
        self.assertTrue(RestClientApis.http_delete_and_get_check(
            dctx_server_urls.dctx_one_state_by_device_id_url.format(mac1_device_id)).success)
        get_resp_object = RestClientApis.http_get_and_check_success(
            dctx_server_urls.dctx_all_states_url)
        states = get_resp_object.json_body['response']['device_states']
    ...
    def test_DctxSecurityGroupPolicy(self):
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                ...
                compare_utils.full_compare_except_keys(['PI_list'])
                ).success
        )
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                dctx_server_urls.dctx_one_state_base_url,
                dumps(dctx_dict),
                DCTX_GET_RESP_SG_DEVOPS).success)
    ...
    def test_RepoPolicy(self):
        ...
        self.assertIs(
            check_validation(
                repo_access_url,
                POLICY_VALIDATION_GET_RESP_DENIED_DCTX_POSTURE_NONCOMPLIANT,
                "Checking Compliant Posture Validation Failure: Non-compliant device"),
            True)
    ...
    def test_Sessions_PolicyTemplate_PolicyContract(self):
        ...
        self.assertTrue(
            RestClientApis.http_post_and_compare_get_resp(
                ...
                compare_utils.full_compare_except_keys(['policy_template_uuid'])
            ).success
        )
        # Principal contract
        self.assertTrue(
        ...
        for i in range(len(request_jsons)):
            ...
            self.assertTrue(RestClientApis.http_delete_and_get_check(url).success)
        PolicyTestCommon.check_no_policy(self)
    ...
    def test_PolicyTemplate_PolicyContract_DeleteSingleContract(self):
        ...
        self.assertTrue(rest_return.success)
        single_contract_url = rest_return.response_object.headers['location']
        self.assertTrue(RestClientApis.http_get_and_compare_resp(
            single_contract_url,
            POLICY_CONTRACT_GET_RESP_FINANCE_FROM_PT_FIRST_FLOOR_NO_PI,
            compare_utils.full_compare_except_keys(['PI_list', 'policy_template_uuid'])
        ).success)
    ...
    def test_SinglePolicyTemplate_SinglePolicyContract_DeleteSingleTemplate(self):
        ...
        rest_resp = RestClientApis.http_post_and_check_success(
            server_urls.policy_template_url,
            POLICY_TEMPLATE_POST_REQ_FINANCE)
        self.assertTrue(rest_resp.success)
    ...
    def test_PolicyContractsFailure_NoTemplate(self):
        ...
        self.assertFalse(RestClientApis.http_post_and_check_success(
            server_urls.policy_contract_url,
            POLICY_CONTRACT_POST_REQ_FINANCE_FROM_PT_BY_NUALA_FOR_ROD
        ).success)
        self.assertTrue(
        ...
        self.assertTrue(
            ptest_check(
                ...
                "Querying for contracts and specific owner")
        )
        # Now we bring the system back to start and
        # check if everything was deleted
        ptest_http_delete_many_and_verify(self, server_urls.policy_templates_url)
    ...
    def test_PutUsers_PutClient_PostContract_GetEntitlements(self):
        ...
        )  # no rest-ut fn yet to check json response on 4xx failure
    ...
    def test_PutUsers_PutClient_LoadContracts_GetEntitlements(self):
        ...
        mc_id = PolicyTestCommon.create_and_check_policy_session(
            self, request_json, resp_json)
        self.assertTrue(load_policies_test())
        entitlements_uri = make_client_all_entitlements_url(mc_id)
    ...
    def test_DeleteAllClients_Delete_allLocationTracking(self):
        ...
        self.assertTrue(rest_return_obj.success)
    ...
    def test_location(self):
        ...
        # make sure we got back a single entitlement
        self.assertEqual(len(entitlements['response']['r_groups']), 1)
        pi_uuid = entitlements['response']['r_groups'][
            0]['environment']['pdp-authorize']['cookie']
        # print(pi_uuid)
        ...
        self.assertTrue(
            RestClientApis.http_put_and_check_success(
                ...
                json.dumps(location_update_dict)).success
        )
        ...
        self.assertTrue(RestClientApis.http_put_and_check_success(
            location_server_update_url,
            json.dumps(location_update_dict)).success
        )
